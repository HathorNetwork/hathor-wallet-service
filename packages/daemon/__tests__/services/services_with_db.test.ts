...
describe('handleTokenCreated (db)', () => {
  ...
  it('should add new token alongside existing token with different token_id from same tx', async () => {
    ...
    const oldTokenId = 'old-token-uid-001';
    const newTokenId = 'new-token-uid-001';
    const tokenName = 'NC Token';
    ...
    expect(oldToken?.name).toBe(tokenName);

    // Verify mapping exists for old token
    let tokensCreated = await db.getTokensCreatedByTx(mysql, txId);
    expect(tokensCreated).toHaveLength(1);
    expect(tokensCreated[0]).toBe(oldTokenId);

    // Now simulate a TOKEN_CREATED event with a new token_id (due to reorg)
    const context = {
      socket: expect.any(Object),
      healthcheck: expect.any(Object),
      retryAttempt: 0,
  ...
  it('should delete old tokens when new TOKEN_CREATED arrives with different first_block', async () => {
    ...
    const oldTokenId1 = 'old-token-001';
    const oldTokenId2 = 'old-token-002';
    ...
    let oldToken2 = await db.getTokenInformation(mysql, oldTokenId2);
    ...
    // Now simulate a TOKEN_CREATED event with a new token_id
    const context = {
      socket: expect.any(Object),
      healthcheck: expect.any(Object),
      retryAttempt: 0,
  ...
  it('should handle TOKEN_CREATED when no existing tokens exist', async () => {
    ...
    const txId = 'nano-tx-fresh';
    const tokenId = 'token-uid-fresh';
    const tokenName = 'Fresh Token';
    const tokenSymbol = 'FRT';

    // Verify no tokens exist for this tx initially
    let tokensCreated = await db.getTokensCreatedByTx(mysql, txId);
    expect(tokensCreated).toHaveLength(0);

    // Simulate a TOKEN_CREATED event
    const context = {
      socket: expect.any(Object),
      healthcheck: expect.any(Object),
      retryAttempt: 0,
  ...
  it('should store first_block when token is created via nano contract', async () => {
    expect.hasAssertions();

    const txId = 'nano-tx-with-block';
    const tokenId = 'token-with-first-block';
    const firstBlock = 'block-hash-123';
    const tokenName = 'Block Token';
    const tokenSymbol = 'BLK';
    ...
    // Verify first_block was stored in token_creation table
    ...
    expect(rows[0].first_block).toBe(firstBlock);
  });

  it('should store null first_block for traditional CREATE_TOKEN_TX tokens', async () => {
    expect.hasAssertions();

    const tokenId = 'create-token-tx-001';
    const tokenName = 'Traditional Token';
    const tokenSymbol = 'TRD';
    ...
    expect(rows).toHaveLength(1);
    expect(rows[0].tx_id).toBe(tokenId); // For CREATE_TOKEN_TX, tx_id = token_id
    expect(rows[0].first_block).toBeNull();
  });

  it('should handle reorg by deleting tokens with old first_block and inserting with new first_block', async () => {
    expect.hasAssertions();

    const txId = 'nano-tx-reorg-blocks';
    const tokenId = 'token-changing-blocks';
    const oldBlock = 'block-old-123';
    const newBlock = 'block-new-456';
    const tokenName = 'Reorg Token';
    ...
    let [rows] = await mysql.query<any[]>(
      'SELECT * FROM `token_creation` WHERE `token_id` = ?',
      [tokenId]
    );
    expect(rows).toHaveLength(1);
    expect(rows[0].first_block).toBe(oldBlock);

    // Simulate token deletion (what would happen when block is voided)
    ...
    // Now simulate TOKEN_CREATED event with new block
    const context = {
      socket: expect.any(Object),
      healthcheck: expect.any(Object),
      retryAttempt: 0,
...
describe('handleVertexAccepted with shielded outputs', () => {
  ...
  it('writes address_balance with kind=shielded for recovered shielded outputs', async () => {
    expect.hasAssertions();
    ...
    const [rows] = await mysql.query<any[]>(
      `SELECT kind, unlocked_balance, locked_balance, total_received, transactions
         FROM address_balance
        WHERE address = ? AND token_id = ?`,
      [shieldedAddress, '00'],
    );
    expect(rows).toHaveLength(1);
    expect(rows[0].kind).toBe('shielded');
    expect(BigInt(rows[0].unlocked_balance)).toBe(150n);
    expect(BigInt(rows[0].locked_balance)).toBe(0n);
    expect(BigInt(rows[0].total_received)).toBe(150n);
...
describe('handleVertexAccepted with shielded spends', () => {
  ...
  beforeEach(async () => {
    // `shielded_address` and `shielded_tx_output_data` are not in the shared
    // cleanDatabase TABLES list; wipe them manually to stay isolated from
    // neighbouring suites.
    await mysql.query('DELETE FROM shielded_tx_output_data');
    await mysql.query('DELETE FROM shielded_address');
  });
  ...
  it('reverses the shielded balance when a shielded UTXO is spent', async () => {
    ...
    // Owned shielded_address row so findShieldedAddressOwnership returns the
    // owning wallet for SHIELDED_ADDRESS.
    await mysql.query(
      `INSERT INTO shielded_address (address, wallet_id, shielded_index, scan_privkey, transactions, created_at)
       VALUES (?, 'wallet_alice', 7, ?, 1, CURRENT_TIMESTAMP)`,
    ...
    // transactions counter ticks up by 1 (1 from seed + 1 from spend).
    ...
    // address_balance (kind=shielded): unlocked_balance decremented to 0;
    // total_received untouched (lifetime credits, not balance).
    //
    // transactions lands at 3, not 2: address_balance's PK is (address,
    // token_id) — `kind` is NOT in the PK — so a spend vertex bumps the
    // shielded row twice. Once by the transparent input bookkeeping in
    // updateAddressTablesWithTx (the shielded input is emitted as a stub
    // input with the same address+token_id and ON DUPLICATE KEY UPDATE
    // collides into the shielded row), and once by the reversal helper.
    // That double-bump is a pre-existing schema gap (PK should include
    // `kind`), tracked separately; not what this test exercises.
    const [addrBalanceRows] = await mysql.query<any[]>(
      `SELECT kind, unlocked_balance, locked_balance,
              total_received, transactions
         FROM address_balance
    ...
    expect(BigInt(addrBalanceRows[0].total_received)).toBe(SEEDED_VALUE);
    expect(Number(addrBalanceRows[0].transactions)).toBe(3);
  ...
  it('does NOT reverse when the spent shielded row is unowned', async () => {
    ...
    // No `shielded_address` ownership row → findShieldedAddressOwnership
    // returns null and the reversal branch is skipped.
    await seedRecoveredShieldedUtxo();
    await seedWalletBalance();
    ...
    // address_balance (kind=shielded) untouched.
    const [addrBalanceRows] = await mysql.query<any[]>(
      `SELECT unlocked_balance, locked_balance
         FROM address_balance
        WHERE address = ? AND token_id = ? AND kind = 'shielded'`,
    ...
    expect(addrBalanceRows).toHaveLength(1);
    expect(BigInt(addrBalanceRows[0].unlocked_balance)).toBe(SEEDED_VALUE);
